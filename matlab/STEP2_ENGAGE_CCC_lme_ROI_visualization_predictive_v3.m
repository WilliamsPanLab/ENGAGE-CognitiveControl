...
roiLabel = 'MixedModel_SCL20_predict';
...
for iRoi = 1:length(roiList)
    ...
    % Prepare fMRI change at 2 mo
    fmri2Mo = repmat(squeeze(taskActivation(:, iRoi, 2)) - squeeze(taskActivation(:, iRoi, 1)), 1, length(includedSessions4Change));
    fmri2Mo = fmri2Mo(:);

    % Overall correlation
    [r, p] = partialcorr(outcomeChange, fmri2Mo, ageSexLong, 'Rows', 'pairwise', 'Type', 'Spearman');
    fitModel = fitlm(fmri2Mo, outcomeChange);
    
    % Group-specific correlations
    [rU, pU] = partialcorr(outcomeChange(groupInfoLong == 0), fmri2Mo(groupInfoLong == 0), ageSexLong(groupInfoLong == 0), 'Rows', 'pairwise');
    [rI, pI] = partialcorr(outcomeChange(groupInfoLong == 1), fmri2Mo(groupInfoLong == 1), ageSexLong(groupInfoLong == 1), 'Rows', 'pairwise');
    
    % Plot scatter plot
    xLabel = ['\Delta Activation in ', roiName];
    yLabel = ['\Delta ', beh_name{outcomeIndex}];
    figPosition = [2, 2, 9.5, 6.35];
    scatterPlot = fmri_behavior_plot4(fmri2Mo, outcomeChange, 1 - groupInfoLong, [183, 90, 101; 166, 166, 172]/255, 1, 0.8, xLabel, yLabel, '', flip(legends), figPosition, '', 0);
