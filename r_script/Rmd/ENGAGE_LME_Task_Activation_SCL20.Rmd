...
```{r}
ROI_names <- c('lAIn','lCaudate','rIPL','rDLPFC', 'rIPL3','lSPL')
```
...
for (i_name in 1:4) {
  ...
  # formula
  formula <-  as.formula(paste("SCL_20_change ~ SCL_20_baseline + BMI_baseline + ", fmri, " + ",fmri,":Time + ",fmri,":Group + ",fmri, ":Time:Group + age + gender + (1|subjects)", sep = ""))
  # # formula that includes medication prescription as a covariate
  # formula <-  as.formula(paste("SCL_20_change ~ SCL_20_baseline + BMI_baseline + ", fmri, " + ",fmri,":Time + ",fmri,":Group + ",fmri, ":Time:Group + med_binary + age + gender + (1|subjects)", sep = ""))
  # # formula that includes behavioral performance as covariates
  # formula <-  as.formula(paste("SCL_20_change ~ SCL_20_baseline + BMI_baseline + ", fmri, " + ",fmri,":Time + ",fmri,":Group + ",fmri, ":Time:Group + fmri_fFA_change + fmri_fFT_change + age + gender + (1|subjects)", sep = ""))
  ...
  #general indicator
  if (i_name < 4) {
    #plot scl ~ fmri x time, with scatters
    formula_plot_1 = as.formula(paste("Time ~ ", fmri, sep = ""));
    print(emmip(lme_model,  formula_plot_1 , cov.reduce = range, CIs = T) +
      geom_point(aes_string(x = fmri, y = "SCL_20_change", colour = "Time", fill = "Time"), data = data_csv, pch = 16, width = 0.1, alpha = 0.5) + labs(title = ROI_name_i, x = "\u0394 NoGo > Go fMRI activation", y = "\u0394 SCL-20") + theme_classic() + theme(legend.position = "top")) #ylim(c(-2.5,2)) + xlim(c(-1.5,1.5)) +
    cat("\n\n\n\n")
  ...
  # time modulation
  if (i_name > 3 && i_name < 5) {
   #plot scl ~ fmri x time, only lines
    formula_plot_1 = as.formula(paste("Time ~ ", fmri, sep = ""));
    print(emmip(lme_model,  formula_plot_1 , cov.reduce = range, CIs = T) + labs(title = ROI_name_i, x = "\u0394 NoGo > Go fMRI activation", y = "\u0394 SCL-20") + theme_classic() + theme(legend.position = "top") + scale_colour_brewer(palette = "Set1") + scale_fill_brewer(palette = "Set1")) #ylim(c(-2.5,2)) + xlim(c(-1.5,1.5)) +
    cat("\n\n\n\n")
    #plot marginal effect
...
### fmri GoNoGo performance vs. SCL-20 
```{r warning=FALSE, paged.print=TRUE, comment= NA, message=FALSE, fig.keep = "all", results='asis'}
...
for (var in variables) {
  ...
  formula <-  as.formula(paste("SCL_20_change ~ SCL_20_baseline + ", var, " + ", var, ":Time + ", var, ":Group + ", var, ":Time:Group + age + gender + (1|subjects)", sep = ""))
  lme_model <- lmer(formula, data = data_csv, REML = FALSE)
  anova_result <- anova(lme_model,type = 'marginal')
  print(anova_result)
